...
int main() {
    if (VIPS_INIT(NULL) != 0) { vips_error_exit("Error initializing VIPS"); }
    ...
    if (input_image == NULL) { vips_error_exit("Error loading input image"); }
    ...
    vips_colourspace(resized_image_4K, &resized_image_4K, VIPS_INTERPRETATION_sRGB, NULL);
    vips_colourspace(resized_image_1K, &resized_image_1K, VIPS_INTERPRETATION_sRGB, NULL);
    vips_colourspace(resized_thumbnail, &resized_thumbnail, VIPS_INTERPRETATION_sRGB, NULL);
    ...
    if (vips_image_write_to_file(
         resized_thumbnail, "output-thumb.avif[Q=50]", NULL) != 0) 
    { vips_error_exit("Error saving AVIF output image"); }
